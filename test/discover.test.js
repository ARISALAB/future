...
const B = require('../benchmarks.js');
const tr = { title: 'TableReserve', h1: 'Online κρατήσεις για το μαγαζί σας, χωρίς κόπο', h2: 'Η εφαρμογή κρατήσεων', desc: '', text: 'TableReserve Online κρατήσεις για το μαγαζί σας. Η πλατφόρμα κρατήσεων για εστιατόρια. Δοκίμασε δωρεάν, δες τα πακέτα συνδρομής. Εγγραφή λογαριασμού. Λογισμικό για τραπέζια και εστιατόρια.' };
assert(C.isSoftware(tr), 'software detected');
assert.strictEqual(C.guessCategory(B, tr), null, 'no restaurant category for software');
const trq = C.buildQuery(tr, 'TableReserve', 'Αθήνα'); assert(/λογισμικό εφαρμογή$/.test(trq) && !/Αθήνα/.test(trq) && /κρατήσ/i.test(trq), 'vendor query: ' + trq);
assert(!C.isSoftware({ title: 'Γέρος του Μωριά – Εστιατόριο Πλάκα', h1: '', h2: '', desc: 'Ταβέρνα με παραδοσιακή κουζίνα', text: 'Κράτηση τραπεζιού. Μενού. Εστιατόριο.' }), 'restaurant is not software');
