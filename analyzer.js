...
(function (root, factory) {
  ...
  function sentenceAt(text, i) {
    ...
    if (end - start > 220) {
      start = Math.max(start, i - 70); end = Math.min(end, i + 130);
      var sp = text.indexOf(' ', start); if (start > 0 && sp >= 0 && sp < i) start = sp + 1;
      var ep = text.lastIndexOf(' ', end); if (ep > i) end = ep;
    }
    return short(text.slice(start, end), 200);
  }
  function sentenceWith(text, re) { var m = re.exec(text); return m ? sentenceAt(text, m.index) : ''; }

  // ---------- λειτουργίες που περιγράφει ένα site (λεξικό· επεκτείνεται εύκολα) ----------
  // g: ομάδα, w: βάρος (3 = βασική λειτουργία του κλάδου, 1 = γενική), re: regex σε κείμενο χωρίς τόνους
  var CAPS = [
  ...
  CAPS.forEach(function (c) { c.re = new RegExp(c.re.source.replace(/ς/g, 'σ')); }); // το κείμενο κανονικοποιείται με τελικό σ
  function detectCaps(mainText, rawText) {
    var nt = normGr(mainText), same = nt.length === mainText.length, out = {};
    CAPS.forEach(function (c) {
      var m = c.re.exec(nt); if (!m) return;
  ...
  function extract(doc, ctx) {
    ...
    if (body) {
      ...
      Array.prototype.slice.call(c2.querySelectorAll('script,style,noscript,template,svg,iframe,nav,header,footer,aside')).forEach(function (n) { if (n.parentNode) n.parentNode.removeChild(n); });
      Array.prototype.slice.call(c2.querySelectorAll('a,li,p,h1,h2,h3,h4,h5,h6,div,span,button,td,th,label,option,section,article,br,dt,dd')).forEach(function (n) { try { n.appendChild(doc.createTextNode(' ')); if (n.parentNode) n.parentNode.insertBefore(doc.createTextNode(' '), n); } catch (e) {} });
      mainText = clean(c2.textContent);
    }
    if (mainText.length < 200) mainText = text;
    var inNav = function (el) { return !!(el.closest && el.closest('nav,header,footer,aside')); };
    var features = [], seenF = {};
    q('h2,h3,h4').forEach(function (h) {
      if (inNav(h)) return;
      var t = spaced(h); if (len(t) < 4 || len(t) > 90 || FEAT_SKIP.test(normGr(t))) return;
      var nx = h.nextElementSibling, d = ''; if (nx && /^(P|DIV|SPAN|UL)$/i.test(nx.tagName)) d = short(spaced(nx), 150);
      var k = normGr(t); if (seenF[k]) return; seenF[k] = 1; features.push({ t: t, d: d });
    });
    q('li').forEach(function (li) {
      if (inNav(li)) return;
      var t = spaced(li); if (len(t) < 18 || len(t) > 140) return;
      var k = normGr(t); if (seenF[k]) return; seenF[k] = 1; features.push({ t: t, d: '' });
    });
    ...
    return {
      ...
      features: features, caps: capsHit,
  ...
  function buildSite(pages, ctx) {
    ...
    var capMap = {}, feats = [];
    pages.forEach(function (p) {
      Object.keys(p.caps || {}).forEach(function (id) { if (!capMap[id]) capMap[id] = { path: pathOf(p.url), quote: p.caps[id] }; });
      (p.features || []).forEach(function (f) { feats.push({ t: f.t, d: f.d, path: pathOf(p.url) }); });
    });
    union.capMap = capMap; union.features = feats;
  ...
  function buildStory(name, score, band, positives, negatives, quick, potential, pageNotes, feat) {
    ...
    var offer = ''; if (feat && feat.length) offer = 'Το site περιγράφει ως βασικά σημεία: ' + feat.slice(0, 3).map(function (f) { return '«' + short(f.t, 60) + '»'; }).join(', ') + '. ';
  ...
  function analyzeSite(pageDocs, ctx) {
    ...
    report.story = buildStory(S.brand, score, band, positives, negatives, quick, potential, report.pages.map(function (x) { return { url: x.url, notes: x.notes }; }), report.func.features);
    return report;
  }
  ...
  function buildFunc(S) {
    var caps = CAPS.filter(function (c) { return S.union.capMap[c.id]; }).map(function (c) { var h = S.union.capMap[c.id]; return { id: c.id, name: c.name, g: c.g, w: c.w, path: h.path, quote: h.quote }; })
      .sort(function (a, b) { return b.w - a.w; });
    return { caps: caps, features: S.union.features.slice(0, 40) };
  }
  function stemsOf(t) { return uniq(normGr(t).match(/\p{L}{5,}/gu) || []).filter(function (w) { return !STOP.has(w); }).map(function (w) { return w.slice(0, 6); }); }
  function featMatch(a, b) {
    var A = stemsOf(a.t + ' ' + a.d), B = stemsOf(b.t + ' ' + b.d);
    var common = A.filter(function (x) { return B.indexOf(x) >= 0; }).length;
    if (common >= 2) return true;
    var at = stemsOf(a.t), bt = stemsOf(b.t);
    return common >= 1 && at.length <= 2 && at.some(function (x) { return bt.indexOf(x) >= 0; });
  }
  function uniqueFeatures(mine, theirs) {
    var out = (mine || []).filter(function (f) { return !(theirs || []).some(function (g) { return featMatch(f, g); }); });
    out.sort(function (a, b) { return ((b.d ? 2 : 0) + (/\d/.test(b.t + b.d) ? 1 : 0)) - ((a.d ? 2 : 0) + (/\d/.test(a.t + a.d) ? 1 : 0)); });
    return out;
  }
  function listNames(arr, n) { return arr.slice(0, n).map(function (c) { return c.name.charAt(0).toLowerCase() + c.name.slice(1); }).join(', ') + (arr.length > n ? ' και άλλες ' + (arr.length - n) : ''); }
  function compareFunctions(me, rival, rivalName) {
    var mc = {}, rc = {};
    me.func.caps.forEach(function (c) { mc[c.id] = c; }); rival.func.caps.forEach(function (c) { rc[c.id] = c; });
    var rOnly = rival.func.caps.filter(function (c) { return !mc[c.id]; }), mOnly = me.func.caps.filter(function (c) { return !rc[c.id]; });
    var both = me.func.caps.filter(function (c) { return rc[c.id]; });
    ...
    var verdict = rOnly.length > mOnly.length + 1 ? 'Με βάση όσα δηλώνουν τα sites, το ' + rivalName + ' καλύπτει περισσότερες λειτουργίες.'
      : mOnly.length > rOnly.length + 1 ? 'Με βάση όσα δηλώνουν τα sites, εσύ καλύπτεις περισσότερες λειτουργίες.' : 'Με βάση όσα δηλώνουν τα sites, καλύπτετε παρόμοιο εύρος λειτουργιών.';
    return { name: rivalName, rOnly: rOnly, mOnly: mOnly, both: both, fR: fR, fM: fM, text: t.join(' '), verdict: verdict };
  }
  function functionsAll(me, rivals, names) {
    var per = rivals.map(function (r, i) { return compareFunctions(me, r, names[i]); });
    var common = per.length ? per[0].rOnly.filter(function (c) { return per.every(function (p) { return p.rOnly.some(function (x) { return x.id === c.id; }); }); }) : [];
    var mine = per.length ? per[0].mOnly.filter(function (c) { return per.every(function (p) { return p.mOnly.some(function (x) { return x.id === c.id; }); }); }) : [];
    var txt = [];
    ...
    return { per: per, common: common, mine: mine, text: txt.join(' ') };
  }
